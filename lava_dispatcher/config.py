...
class DeviceSchema(schema.Schema):
    ...
    boot_cmd_timeout = schema.IntOption(default=30)
    ...
    fvp_terminal_port_pattern = schema.StringOption(
        default="terminal_0: Listening for serial connection on port (\d+)")
...
def _lookup_ip(lava_network_iface):
    ...
    raise CommandError("LAVA_NETWORK_IFACE is set to '%s' "
                       "but no IP address was found for any listed interface."
                       % ", ".join(lava_network_iface))
...
def get_config():
    ...
    if config.lava_network_iface:
        ...
        config.lava_image_url = cp.get('__main__', "LAVA_IMAGE_URL",
                                       vars={'LAVA_SERVER_IP': config.lava_server_ip})
    return config

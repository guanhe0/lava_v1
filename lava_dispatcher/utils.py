...
def extract_rootfs(rootfs, root):
    ...
    if logging_system('nice tar --selinux -C %s -xaf %s' % (root, rootfs)):
        logging.warning('Unable to extract tarball: %s ' % rootfs)
        ...
        if logging_system('nice tar --selinux --strip-components=1 -C %s -xaf %s' % (root, rootfs)):
            raise CriticalError('Unable to extract tarball: %s with --strip-componets' % rootfs)
...
class logging_spawn(pexpect.spawn):
    ...
    def sendline(self, s='', delay=12, send_char=True):
        """
    ...
    def send(self, string, delay=12, send_char=True):
    ...
    while retry_count < retry_limit:
        try:
            ...
            for pattern, result in expectations.items():
                patterns.append(pattern)
                results.append(result)
            lines = context.device_config.connection_command.split(";")
            ...
            expectations.pop(hostname)
            patterns.remove(hostname)
            results.remove('continue')
            #proc = context.spawn(
